  ...
  methods: {
    ...
    readConfig() {
      ...
      if (this.data.config) {
        ...
        this.setData({
          ...
          hourColumn: conf.column == "hour" || conf.column == "min" || conf.column == "sec",
          minColumn: conf.column == "min" || conf.column == "sec",
          secColumn: conf.column == "sec",
          noColumn: conf.column == ""
        });
      }
    ...
    onConfirm: function() {
      ...
      if (startTime <= endTime || !this.data.endDate) {
        ...
        let startTimeBack = `${startArr[0]}-${format0(startArr[1])}-${format0(startArr[2])} ${(this.data.hourColumn ? format0(startArr[3]) : this.data.noColumn ? "" : "00")}${this.data.noColumn ? "" : ":"}${(this.data.minColumn ? format0(startArr[4]) : this.data.noColumn ? "" : "00")}${this.data.noColumn ? "" : ":"}${(this.data.secColumn ? format0(startArr[5]) : this.data.noColumn ? "" : "00")}`
        ...
        let endTimeBack =
          ...
          format0(endArr[2]) +
          " " +
          (this.data.hourColumn ? format0(endArr[3]) : this.data.noColumn ? "" : "00") +
          `${this.data.noColumn ? "":":"}` +
          (this.data.minColumn ? format0(endArr[4]) : this.data.noColumn ? "" : "00") +
          `${this.data.noColumn ? "":":"}` +
          (this.data.secColumn ? format0(endArr[5]) : this.data.noColumn ? "" : "00");

        let time = {
          startTime: startTimeBack.trim(),
          endTime: endTimeBack.trim()
        };
    ...
    initPick: function() {
      ...
      const nowDay = this.data.rockbock ? date.getDate() - 1 : date.getDate() + 1;
      ...
      const startDay = this.data.rockbock ? startDate.getDate() - 1 : startDate.getDate() + 1;
    ...
    setStartDate: function(year, month, day, hour, minute, second) {
      let pickerDateArr = this.setPickerDateArr(
        ...
        year,
        month,
        this.data.rockbock ? day - 1 : day,
        hour,
        minute,
        second
      );

      this.data.rockbock = true;
...
const formatTime = date => {
  typeof date == 'string' || 'number' ? date = new Date(date) : console.error("标准化时间不对!");
